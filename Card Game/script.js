...
*/ /* // Instructions Create a new function (getCard) // Inside the function // Create random index // Grab a random card from the cards array // 

create new image element // set the 'src' attribute of the img use // setAttribute() // append the image the the deck // Add the function to the draw one buttons
...
*/
const drawOne = document.getElementById("draw-card")
const drawFive = document.getElementById("pick-five")
...
const getCard = () => {
    ...
    newImg.setAttribute('src', CARDS[num].image )
    
    console.log(CARDS[num])
    console.log(deckOfCards)
    deckOfCards.appendChild(newImg)
    return CARDS[num]
...
const reset = () => {
  while(deckOfCards.firstChild) {
    deckOfCards.removeChild(deckOfCards.lastChild);

    }
}

drawOne.addEventListener("click", getCard)
drawFive.addEventListener("click", drawFiveCards )
